...
def do_detect(model, img, conf_thresh, nms_thresh, use_cuda=1, gpu_number=0):
    ...
    if type(img) == np.ndarray and len(img.shape) == 3:  # cv2 image
        ...
        temp = img.transpose(0, 3, 1, 2)
        t1_temp = time.time()
        img = torch.from_numpy(temp).float().div(255.0)
        t2_temp = time.time()
    else:
    ...
    # print('-----------------------------------')
    # print('           Preprocess_temp_1 : %f' % (t1_temp - t0))
    # print('           Preprocess_temp_2 : %f' % (t2_temp - t0))
    # print('           Preprocess_temp_3 : %f' % (t3_temp - t0))
