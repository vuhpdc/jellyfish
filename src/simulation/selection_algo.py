...
def ModelSelectionSimulatedAnnealing(simData, initialModelsInfo, clientsInfo, MappingAlgo, debug=False,
    ...
    def shuffleModelList(initialModelsInfo, newModelsInfo, newMappingInfo):
        ...
        shuffledMappingInfo = env.MappingInfo(shuffledModelsInfo, newMappingInfo.clients_info,
        ...
        while not stopCondition(bestMappingInfo, action, temparature, iter, iterWithoutSol):
            ...
            if (betterMapping(newMappingInfo, bestMappingInfo, action)):
                if action == DEGRADE:
                    ...
                    print("Found better solution: accuracy before {:.2f} and after {:.2f}".format(bestMappingInfo.metrics.accuracy_per_request,
                                                                                                  newMappingInfo.metrics.accuracy_per_request,
                                                                                                  ))
                assert newMappingInfo.metrics.accuracy_per_request >= 0.0
    ...
    # Again reshuffle the model list. This is because to increases the chances of
...
def ModelSelectionCplexMilp(simData, initialModelsInfo, clientsInfo, MappingAlgo, debug=False):
    from docplex.mp.model import Model as MpModel
    from docplex.util.environment import get_environment
    from docloud.status import JobSolveStatus

    def extract_solution(msol, x_decision):
        clients_map = {}
        models_map = {}
        for client in clients:
            clients_map[client.id] = None

        for client in ineffective_clients:
            clients_map[client.id] = None

        if not msol:
            for model_id, model in initialModelsInfo.items():
                models_map[model_id] = []
            return env.MappingInfo(initialModelsInfo, clientsInfo, models_map, clients_map)

        modelsInfo = {}
        for i in range(total_clients):
            client = clients[i]
            for k in range(total_gpus):
                for j in range(total_models):
                    for b in range(batch_size):
                        value = x_decision[k, i, j, b].solution_value
                        value = int(round(value))
                        model_id = env.DNNModel.getIdString(j, k)

                        if value == 1:
                            model_id = env.DNNModel.getIdString(j, k)
                            if model_id not in modelsInfo:
                                model = env.DNNModel(simData,
                                                     model_number=j, gpu_number=k)
                                assert(model_id == model.id)
                                modelsInfo[model.id] = model
                                models_map[model.id] = []
                            else:
                                model = modelsInfo[model_id]

                            clients_map[client.id] = model
                            models_map[model.id].append(client)

                            assigned = model.check_and_assign(client)
                            # assert assigned, "Client cannot be assigned to Model"
                            if assigned is False:
                                print(
                                    "Client {} cannot be assigned to Model {} on GPU {} with batch size {}".format(c, j, k, b))
                                print("Model Info: {}, {}".format(
                                    latency_mat[j, b], throughput_mat[j, b]))

                                assert False

        return env.MappingInfo(modelsInfo, clientsInfo, models_map, clients_map)

    clients = []
    ineffective_clients = []
    for client in clientsInfo.values():
        if client.lat_budget[0]:
            clients.append(client)
        else:
            ineffective_clients.append(client)

    # consider only effective clients
    total_clients = len(clients)

    mdl = MpModel()
    # mdl.add_parameters(LogVerbosity=CpParam.VALUE_QUIET)
    # mdl.add_parameters(WarningLevel=0)
    mdl.parameters.threads = 4
    mdl.parameters.mip.tolerances.mipgap = 0.000001
    mdl.parameters.timelimit = 60 * 30

    # Get constant data
    total_models = simData.num_models
    total_gpus = simData.num_gpus
    batch_size = simData.max_batch_size
    throughput_mat = np.zeros(
        (total_models, batch_size), dtype=float)
    latency_mat = np.zeros(
        (total_models, batch_size), dtype=float)
    accuracy_vec = np.zeros(total_models, dtype=float)

    for j in range(total_models):
        throughput_mat[j] = simData.throughput_m[j]
        latency_mat[j] = simData.lat_m[j]
        accuracy_vec[j] = simData.acc_m[j]

    fps_vec = np.zeros(total_clients, dtype=int)
    lat_budget_mat = np.zeros((total_clients, total_models), dtype=float)
    for i in range(total_clients):
        fps_vec[i] = clients[i].fps
        for j in range(total_models):
            lat_budget_mat[i][j] = clients[i].lat_budget[j]

    # Define decision variables
    # x_decision for user to gpu mapping
    x_decision = []
    for g in range(total_gpus):
        x_c = []
        for c in range(total_clients):
            x = [[mdl.binary_var(name="UserMapping_" + str(g) + "_" + str(c) + "_" + str(m) + "_" + str(b))
                  for b in range(batch_size)] for m in range(total_models)]
            x_c.append(x)
        x_decision.append(x_c)

    # z_decision for GPU to batch size
    z_decision = []
    for g in range(total_gpus):
        z = [[mdl.binary_var(name="Batching_" + str(g) + "_" + str(m) + "_" + str(b))
              for b in range(batch_size)] for m in range(total_models)]
        z_decision.append(z)

    x_decision = np.array(x_decision)
    z_decision = np.array(z_decision)

    # A. Uniqueness constraint
    # 1. Client map to only one GPU
    for i in range(total_clients):
        ctname = "Clients_{}".format(i)
        mdl.add_constraint(mdl.sum(x_decision[:, i, :, :]) <= 1, ctname=ctname)

    # 2. Atmost one model selected for one GPU
    for k in range(total_gpus):
        ctname = "GPUs_{}".format(k)
        mdl.add_constraint(mdl.sum(z_decision[k, :, :]) <= 1, ctname=ctname)

    for k in range(total_gpus):
        for i in range(total_clients):
            for j in range(total_models):
                for b in range(batch_size):
                    ctname = "XZEqual_{}_{}_{}_{}".format(k, i, j, b)
                    mdl.add_constraint(
                        z_decision[k, j, b] >= x_decision[k, i, j, b], ctname=ctname)

    # B. Compute helper variables
    # 1. Per GPU latency and throughput calculation
    throughput_per_gpu = []
    for k in range(total_gpus):
        throughput = 0
        for j in range(total_models):
            for b in range(batch_size):
                throughput += (z_decision[k, j, b]
                               * throughput_mat[j, b])

        throughput_per_gpu.append(throughput)
        mdl.add_kpi(throughput, "Throughput on GPU " + str(k))

    # 2. Per Client latency budget and accuracy
    lat_budget_per_client = []
    acc_per_client = []
    for i in range(total_clients):
        lat_budget = 0
        acc = 0
        ...
        for k in range(total_gpus):
            ...
            for j in range(total_models):
                for b in range(batch_size):
                    lat_budget += x_decision[k, i, j, b] * lat_budget_mat[i, j]
                    latency_per_gpu += x_decision[k,
                                                  i, j, b] * latency_mat[j, b]

                    acc += x_decision[k, i, j, b] * accuracy_vec[j]
            latency_per_gpu_lst.append(latency_per_gpu)

        lat_budget_per_client.append(lat_budget)
        # 4. Latency constraint
        for k in range(total_gpus):
            ctname = "LatBudget_{}_{}".format(i, k)
            mdl.add_constraint(lat_budget >= 2 *
                               latency_per_gpu_lst[k], ctname=ctname)
        acc_per_client.append(acc)
        mdl.add_kpi(acc, "Acc for client " + str(i))
    ...
    for k in range(total_gpus):
        ...
        for i in range(total_clients):
            for j in range(total_models):
                for b in range(batch_size):
                    rate_assigned += (x_decision[k, i, j, b] * fps_vec[i])

        ctname = "GPUThroughput_{}".format(k)
        mdl.add_constraint(
            rate_assigned <= throughput_per_gpu[k], ctname=ctname)
        mdl.add_kpi(rate_assigned, "Rate on GPU " + str(k))

    # 6. Latency budget constraint
    for i in range(total_clients):
        ctname = "LatBudgetZero_{}".format(i)
        mdl.add_constraint(
            lat_budget_per_client[i] >= 1, ctname=ctname)
        mdl.add_kpi(lat_budget_per_client[i],
                    "Latency budget of Client " + str(i))

    for k in range(total_gpus):
        latency = 0
        for j in range(total_models):
            for b in range(batch_size):
                latency += z_decision[k, j, b] * latency_mat[j, b]

        mdl.add_kpi(latency, "Latency on GPU {}".format(k))

    # Objective
    obj = 0
    total_rate = 0
    for i in range(total_clients):
        obj += acc_per_client[i] * fps_vec[i]
        total_rate += fps_vec[i]
    ...
    # msol = mdl.solve(agent='local',
    # execfile='/home/vinod/packages/cplex_studio1210/cpoptimizer/bin/x86-64_linux/cpoptimizer')
    # msol = mdl.solve(log_output=True)
    msol = mdl.solve(log_output=False)

    if msol and debug:
        print('{:=^100}'.format('Cplex mapping solution'))
        print("x_decision")

        with get_environment().get_output_stream("cplex_mapping.json") as fp:
    ...
    if mdl.get_solve_status() != JobSolveStatus.OPTIMAL_SOLUTION:
        msol = False

    mappingInfo = extract_solution(msol, x_decision)
    return mappingInfo, [mappingInfo]
